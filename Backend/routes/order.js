...
router.post('/', async (req, res) => {
  try {
    ...
    const finalPaymentStatus = payment_status || (payment_method === 'cod' ? 'pending' : 'completed');
    ...
    const params = [
      ...
      delivery_charge, group_discount, total_amount, status, finalPaymentStatus,
      payment_method, payment_id, delivery_address, delivery_date, notes,
      customerDetailsJson
    ];
...
// Update order payment status (for COD completion)
router.put('/:orderId/payment', async (req, res) => {
  try {
    const { orderId } = req.params;
    const { payment_status, payment_id, notes } = req.body;
    ...
    const sql = `
      UPDATE orders 
      SET payment_status = ?, payment_id = ?, notes = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `;

    const result = await query(sql, [payment_status, payment_id, notes, orderId]);
